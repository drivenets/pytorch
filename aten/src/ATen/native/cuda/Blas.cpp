...
#ifdef USE_ROCM
  // ROCm hipBLASLt supports more flexible layouts than cuBLASLt
  // Allow different layout combinations for better FP8 performance
  TORCH_CHECK(true, "hipBLASLt supports flexible layouts");
#else
  TORCH_CHECK(args.transa == 't' && args.transb == 'n', "Only multiplication of row-major and column-major matrices is supported by cuBLASLt");
#endif

#ifdef USE_ROCM
  // Check if we can use optimal algorithm selection (9 algorithms vs 1-2)
  bool use_optimal_selection = false;
  if (!bias.has_value() && scale_a.numel() == 1 && scale_b.numel() == 1) {
    // Check if scales are unity (1.0) - allows optimal kernel selection
    float scale_a_val = scale_a.item<float>();
    float scale_b_val = scale_b.item<float>();
    if (scale_a_val == 1.0f && scale_b_val == 1.0f) {
      use_optimal_selection = true;
    }
  }
  
  if (use_optimal_selection) {
    // Use direct hipBLASLt call with minimal attributes for maximum algorithm selection
    printf("DEBUG: Using optimal FP8 algorithm selection path\n");
    
    // Implement direct hipBLASLt call for optimal performance
    hipblasLtHandle_t handle = at::cuda::getCurrentCUDABlasLtHandle();
    
    // Create minimal matrix layouts
    hipblasLtMatrixLayout_t layoutA, layoutB, layoutC;
    hipblasLtMatrixLayoutCreate(&layoutA, HIP_R_8F_E4M3_FNUZ, args.m, args.k, args.lda);
    hipblasLtMatrixLayoutCreate(&layoutB, HIP_R_8F_E4M3_FNUZ, args.k, args.n, args.ldb);
    hipblasLtMatrixLayoutCreate(&layoutC, HIP_R_16F, args.m, args.n, args.result_ld);
    
    // Create minimal matmul descriptor (no bias, no scaling pointers)
    hipblasLtMatmulDesc_t matmulDesc;
    hipblasLtMatmulDescCreate(&matmulDesc, HIPBLAS_COMPUTE_32F, HIP_R_32F);
    
    // Only set basic transpose attributes
    hipblasOperation_t opA = (args.transa == 't' || args.transa == 'T') ? HIPBLAS_OP_T : HIPBLAS_OP_N;
    hipblasOperation_t opB = (args.transb == 't' || args.transb == 'T') ? HIPBLAS_OP_T : HIPBLAS_OP_N;
    ...
    // Create preference with large workspace
    ...
    // Get algorithms (should find 9+ algorithms)
    hipblasLtMatmulHeuristicResult_t heuristic[10];
    int returned_results = 0;
    hipblasStatus_t algo_status = hipblasLtMatmulAlgoGetHeuristic(
        handle, matmulDesc, layoutA, layoutB, layoutC, layoutC,
        preference, 10, heuristic, &returned_results
    );
    
    if (returned_results > 0) {
      printf("DEBUG: Found %d optimal algorithms (vs 1 in complex path)\n", returned_results);
      ...
      hipblasStatus_t exec_status = hipblasLtMatmul(
          handle, matmulDesc, &alpha,
          args.mata->data_ptr(), layoutA,
          args.matb->data_ptr(), layoutB,
          &beta,
          args.result->data_ptr(), layoutC,
          args.result->data_ptr(), layoutC,
          &heuristic[0].algo,
      ...
      if (exec_status == HIPBLAS_STATUS_SUCCESS) {
        printf("DEBUG: Optimal FP8 path succeeded!\n");
        return out; // Skip the complex tunable path
      }
    }
    
    printf("DEBUG: Optimal path failed, falling back to complex path\n");
  }
#endif
